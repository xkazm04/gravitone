...
cmd_bench() {
  ...
  # `gravitone` is the CLONE directory; `gravitone/gravitone` is the application
  # root inside it (the repo carries the arm/ workspace, not the app alone).
  _send "$id" "'set -e; sudo -u ubuntu bash -lc \"cd ~ && (git -C gravitone pull -q || git clone -q $REPO) && cd gravitone/gravitone && bash benchmark_arm.sh 2>&1 | tail -c 20000\"'"
}
