...
# The repository root is NOT the application root: the repo holds the arm/
# workspace, and the service lives in the gravitone/ subdirectory. CLONE_DIR is
# what git writes; APP_DIR is what everything downstream (the unit helper, the
# docker build context) must point at. Collapsing the two back into one path
# builds an image from a directory with no Dockerfile in it.
CLONE_DIR=/opt/gravitone
APP_DIR="$CLONE_DIR/gravitone"
...
# --- code -------------------------------------------------------------------
if [ -d "$CLONE_DIR/.git" ]; then git -C "$CLONE_DIR" pull -q; else git clone -q "$REPO" "$CLONE_DIR"; fi
[ -f "$APP_DIR/Dockerfile" ] || { echo "!! $APP_DIR has no Dockerfile — the repo layout moved; check CLONE_DIR/APP_DIR"; exit 1; }
