...
- `context-map.json` - 21 contexts in 8 groups (speech synthesis, capacity and admission,
...
- `deploy/README.md` - measure, plan, deploy: the chart's defaults are one box's numbers, and
  `python -m service.plan` writes the overlay for other silicon.
- `docs/harness/harness-learnings.md` - how the suite really runs: `python -m unittest
